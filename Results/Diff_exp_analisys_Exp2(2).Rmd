...
eval_samples <- metadata  %>% filter(Run.isin(pair_metadata))
...
avaible = length(eval_files)
avaible
# sampleTable <- data.frame(condition = factor(eval_samples$category[1:avaible]))
sampleTable <- read.csv(paste0("/home/karen/Documents/phd/DifferentialExpression/",experiment,"/metadata_filtered.csv"), header = TRUE)
...
something <- something[order(something$padj),]
